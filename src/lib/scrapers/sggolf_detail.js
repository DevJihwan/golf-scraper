...
export async function scrapeSggolfDetail(logInfo, logWarn, logError) {
    const scraperId = 'sggolf_detail';
    logInfo(`=== ${scraperId} 스크래핑 시작 ===`);
    setScraperStatus(scraperId, 'running');

    // 설정
    const BASE_URL = 'https://www.sggolf.com/shop';
    const INPUT_FILE = path.join(process.cwd(), 'data', 'stores', 'sggolf_shops.json');
    const OUTPUT_FILE = path.join(process.cwd(), 'data', 'stores', 'sggolf_stores_detailed.json');
    ...
    try {
        for (let shopIndex = currentShopIndex; shopIndex < shops.length; shopIndex++) {
            ...
            try {
                ...
                const response = await axios.post('https://www.sggolf.com/main/ajax_shop_detail', postData, {
                    headers: {
