// src/lib/scrapers/friendgolf_details.js
...
/**
 * FriendGolf 상세 스크래퍼 함수
 * @param {Function} logInfo - 정보 로그 함수
 * @param {Function} logWarn - 경고 로그 함수
 * @param {Function} logError - 오류 로그 함수
 */
export async function scrapeFriendgolfDetails(logInfo, logWarn, logError) {
    const scraperId = 'friendgolf_details';
    logInfo(`=== ${scraperId} 스크래핑 시작 ===`);
    setScraperStatus(scraperId, 'running');

    // 설정
    const BASE_URL = 'https://www.friendsscreen.kr/main/shop';
    const INPUT_FILE = path.join(process.cwd(), 'data', 'stores', 'friendgolf_shops.json');
    const OUTPUT_FILE = path.join(process.cwd(), 'data', 'stores', 'friendgolf_stores_detailed.json');
    ...
    try {
        for (let shopIndex = currentShopIndex; shopIndex < shops.length; shopIndex++) {
            ...
            try {
                ...
                const response = await axios.post('https://www.friendsscreen.kr/main/ajax_shop_detail', postData, {
                    headers: {
...
export default scrapeFriendgolfDetails;
